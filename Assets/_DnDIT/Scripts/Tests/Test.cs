...
public class Test : MonoBehaviour
{
    ...
    public void OnImagePressed()
    {
        ...
        NativeGallery.GetImageFromGallery(path =>
        {
            ...
            NativeGallery.SaveImageToGallery(path, "DnDIT", "loadedImage", (success, s) =>
            {
                imagePath.text = $"Saving to {s} - {success}";
            } );
        });
    }
    ...
    IEnumerator LoadAudioClip(string path)
    {
        ...
        {
            audioPath.text = $"Error loading audio: {www.error} - Uri path: {uri.AbsolutePath} - AudioType: {audioType}";
        }
        else
        {
            ...
            if (_audioClip != null)
            {
                ...
                NativeGallery.SaveAudioToGallery(path, "DnDIT", "loadedAudioClip", (success, s) =>
                {
                    imagePath.text = $"Saving to {s} - {success}";
                } );
            }
            else
            {
                ...
                audioPath.text = $"Failed to get AudioClip from downloaded content. - path {path} - Uri path: {uri.AbsoluteUri} - www {www.result} - AudioType: {audioType}";
            }
        }
    }
